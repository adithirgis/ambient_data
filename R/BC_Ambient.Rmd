...
p1<-ggplot(BC_melt, aes(date, BC, color=Instrument))+
  labs(x="",  y= expression(paste(BC[2.5] ," (", mu, "g",~m^{-3}, ")")),title=paste0(""))+ scale_y_continuous(limits = c(0,75), expand = c(0, 0))+scale_x_datetime(date_breaks = "1 month", date_labels = "%b %y")+scale_color_uchicago() +
  ...
  geom_hline(aes(yintercept = 60), color = "red", size = 1)+geom_line(size=1.2)+annotate("text", x = as.POSIXct("2019-07-30 21:00:00"), y = 65, family = "Poppins", size = 5, color = "red",label = "National Standard")+annotate("text", x = as.POSIXct("2019-07-30 21:00:00"), y = 65, family = "Poppins", size = 5, color = "red",label = "National Standard")
p1
```
...
```{r   message=FALSE, warning=FALSE}
print(paste0("The t-test resulted in p value = ", round(t_test$p.value, digits = 5), ", mean of LC_AE51_ILK = ", round(t_test$estimate[1], digits = 2), " and mean of AE33_CSTEP = ", round(t_test$estimate[2], digits=2)))
```
